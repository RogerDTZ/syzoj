...
app.get("/contest/:id/edit", async (req, res) => {
  try {
    ...
    if (contest.contestants) 
      contestants = await contest.contestants
        .split('|')
        .mapAsync(async (id) => await User.findById(id));
    if (contest.contestant_groups) 
      usergroups = await contest.contestant_groups
        .split('|')
        .mapAsync(async (id) => await UserGroup.findById(id));
      ...
      for (let problem of problems) {
        if (contest.type === "noi") {
          if (player.score_details[problem.problem.id]) {
            ...
            if (
              !contest.ended &&
              !(await problem.problem.isAllowedEditBy(res.locals.user)) &&
          ...
          if (player.score_details[problem.problem.id]) {
            ...
            await contest.loadRelationships();
            let multiplier =
              contest.ranklist.ranking_params[problem.problem.id] || 1.0;
            problem.feedback =
  ...
  try {
      ...
      if (req.body.language === "submit-answer") {
        query.andWhere(
          new TypeORM.Brackets((qb) => {
            qb.orWhere("language = :language", { language: "" }).orWhere(
              "language IS NULL"
            );
          })
        );
    ...
    res.render("submissions", {
      ...
      items: judge_state.map((x) => ({
        ...
        token:
          getRoughResult(x, displayConfig) == null && x.task_id != null
            ? jwt.sign(
                {
                  taskId: x.task_id,
                  type: pushType,
...
app.get("/contest/submission/:id", async (req, res) => {
  try {
    ...
    res.render("submission", {
      ...
      socketToken:
        displayConfig.showDetailResult && judge.pending && judge.task_id != null
          ? jwt.sign(
              {
